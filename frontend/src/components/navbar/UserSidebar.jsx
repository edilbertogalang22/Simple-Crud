...
import useSidebarMenu from "../../hooks/useUserSidebarMenu";
                ...
                return (
                  <li key={item.path}>
                    {item.isLogout ? (
                      <button
                        onClick={() => {
                          handleLogout(); // call your custom hook logout
                          handleClose(); // close sidebar on mobile
                        }}
                        className={`${linkStyle({ isActive: false })} flex items-center gap-3 p-3 w-full text-left text-blue-500 rounded hover:bg-gray-700 transition`}
                      >
